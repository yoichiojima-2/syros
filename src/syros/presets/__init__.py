...
catalog skills of which the workspace installs both — one general, one narrowed
to that workspace — so the install model is visible.
...
CATALOG: tuple[Preset, ...] = (
    Preset(
        ...
        spec={
            ...
            "options": {
                ...
                "max_budget_usd": 5.0,
                # installs from the catalog: sessions under this workspace mount
                # exactly these two, whatever settings/global installs
                "skills": ["brief", "research-brief"],
            },
    ...
    Preset(
        ...
        kind="skill",
        description=(
            "Catalog skill: how to write a decision-ready brief. Two files, so it also shows"
            " what a skill with a reference/ directory looks like. The research workspace"
            " installs it; install it on settings/global to mount it everywhere."
        ),
        spec={"name": "brief", "files": "skills/brief"},
    ),
    Preset(
        name="research-brief",
        kind="skill",
        description=(
            "The same idea narrowed to the research workspace's house style. Installed only"
            " there, so the pair makes the model visible in the console: one catalog, and each"
            " target mounting the skills it installs."
        ),
        spec={"name": "research-brief", "files": "skills/research-brief"},
    ),
    Preset(
        name="researcher",
        kind="agent",
...
class _Present:
    ...
    same question and one catalog listing answer the same question, and gather
    cleanly.
    """
...
async def _present(chosen: list[Preset], *, store: Any, objects: Any) -> _Present:
    agent_docs, workflow_docs, workspace_docs, dirs, skill_stats = await asyncio.gather(
        store.list_agents(),
...
async def _write_files(preset: Preset, *, store: Any, objects: Any, force: bool) -> tuple[int, int]:
    ...
    listed = await (objects.skill_files(name) if skill else objects.workspace_files(name))
    ...
    for file, data in pending:
        if skill:
            await objects.write_skill_file(name, file, data)
        else:
