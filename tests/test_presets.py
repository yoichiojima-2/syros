...
async def test_install_creates_every_kind():
    ...
    assert set(objects.skills) == {"brief", "research-brief"}
...
async def test_the_two_brief_skills_are_distinct_catalog_entries():
    """One catalog, two names — the narrowed one is installed by the workspace,
    not by living somewhere else."""
    _summary, store, objects = await install()
    assert objects.skills["brief"]["SKILL.md"] != objects.skills["research-brief"]["SKILL.md"]
    assert set(objects.skills["brief"]) == {"SKILL.md", "reference/structure.md"}
    # the demo is the install list, so the workspace has to carry it
    assert store.workspaces["research"]["options"]["skills"] == ["brief", "research-brief"]
...
async def test_install_reports_files_written():
    ...
    written = (
        ...
        + len(objects.skills["research-brief"])
    )
...
async def test_status_tracks_each_catalog_skill_separately():
    store, objects = FakeStore(), FakeObjects()
