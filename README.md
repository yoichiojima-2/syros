...
into every run) and per-workspace (`workspaces/{name}/skills/`, mounted only for that
workspace's sessions, shadowing a same-named global). The console has a Skills view with the same
