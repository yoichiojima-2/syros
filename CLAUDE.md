...
- `AgentOptions` (`options.py`) is the serializable subset of ClaudeAgentOptions;
  ...
  explicit ← agent ← team ← `settings/global` ← model `"sonnet"` floor; the merged
  result is snapshotted onto the session.
- Teams (`teams.py`, `teams/{name}` in Firestore) absorb the old workspaces: one
  doc holds stored option defaults AND the exclusive workspace lease. The shared
  directory keeps the `workspaces/{name}/` GCS prefix. A team's `CLAUDE.md` sits at
  that root and loads as project memory (runner passes
  `setting_sources=["user", "project"]`).
- Skills are GCS prefixes, two scopes: `skills/` (global, mounted everywhere) and
  `team-skills/{team}/` (mounted for that team, shadows same-named globals).
